  ...
  return (
    <section className="bg-white px-6 py-20 sm:px-10 sm:py-24 lg:px-14 lg:py-28">
      <div className="mx-auto w-full max-w-[112rem]">
        <h2 className="mb-12 text-3xl font-medium tracking-tight text-neutral-950">
          Now trending
        </h2>

        {isPending && <TrendingSkeleton />}

        {isError && (
          <p className="text-sm text-destructive">
            {getErrorMessage(error, "Failed to load trending products")}
          </p>
        )}
    ...
    >
      <div className="relative h-24 w-full overflow-hidden bg-white">
        {product.imageUrl ? (
          ...
          <div className="flex h-full items-center justify-center bg-muted text-xs font-medium uppercase tracking-wider text-muted-foreground">
            Luxian
          </div>
        )}
      </div>
      <div className="min-w-0">
        <p className="truncate text-base font-medium text-neutral-950">
          {product.category?.name ?? "Luxian"}
        </p>
        <p className="mt-1 line-clamp-2 text-base leading-snug text-neutral-500">
          {product.name}
        </p>
      </div>
    </Link>
